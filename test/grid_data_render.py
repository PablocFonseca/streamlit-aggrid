...
if 1 in TESTS:
    """grid renders lists"""
    test_data = pd.DataFrame({"names": ["alice", "bob"], "ages": [25, 30], "list":[[1,2,3], [4,5,6]] })
...
if 10 in TESTS:
    ...
    AgGrid(unhashable_lists, key="test_unhashable_lists", use_json_serialization='auto')  # Should automatically fallback
...
if 13 in TESTS:
    ...
    AgGrid(complex_unhashable, key="test_complex_unhashable", use_json_serialization=True)
...
if 14 in TESTS:
    ...
    AgGrid(mixed_data, key="test_mixed_data", use_json_serialization=True)
